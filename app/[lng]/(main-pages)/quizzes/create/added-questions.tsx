                    ...
                    if (question.type === "CREATE") {
                        return (
                            ...
                            >
                                ...
                                {/*<p className="text-xs text-gray-400">*/}
                                {/*    Topics: {question.topicId.length > 0 ? question.topicId.join(", ") : "None"}*/}
                                {/*</p>*/}
                        ...
                        // Рендер для вопроса типа GENERATE (функциональность генерации пока отключена)
                        return (
                            <div
                                key={`question${index}`}
                                className="flex flex-col border border-dashed border-gray-500 rounded-md p-3 relative w-[calc(100%-8px)] first:mt-2"
                            >
                                <h2>Generated Question</h2>
                                <p className="text-xs text-gray-400">(This question will be generated automatically)</p>
                                <button
                                    onClick={() => dispatch(removeQuestion(index))}
                                    className="rounded-full bg-red-600 text-white flex items-center justify-center w-4 h-4 -top-2 -right-2 absolute"
                                >
