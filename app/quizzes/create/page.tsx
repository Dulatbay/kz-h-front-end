...
export default function CreateQuiz() {
  ...
  const questions = useSelector(
    (state: RootState) => state.quizOptions.questions
  )
  ...
  function createQuiz() {
    ...
    const previewCheckbox = document.getElementById(
      'previewCheckbox'
    ) as HTMLInputElement
  ...
  async function sendQuiz() {
    ...
    let description = useSelector(
      (state: RootState) => state.quizOptions.description
    )
    let showQuestions = useSelector(
      (state: RootState) => state.quizOptions.showQuestions
    )
    ...
    <div className="w-full max-w-[1200px] mx-auto flex flex-col px-8 items-center">
      ...
      <div className="w-full flex gap-3">
        <div className="w-1/2 flex flex-col gap-4">
          <input
            id="titleInput"
            className="h-10 bg-[#FFFFFF24] w-full text-[#91898C] rounded-md pl-3"
            type="text"
            placeholder="Title"
          />
          <textarea
            id="descArea"
            className="h-40 bg-[#FFFFFF24] w-full text-[#91898C] rounded-md pl-3"
          ...
          <div className="flex gap-2">
            ...
            >
              Show questions before start
            </label>
          </div>
          <Button
            className={`w-full h-12 ${styles.button_primary}`}
            onClick={createQuiz}
          >
            Create Quiz
          </Button>
          ...
          return (
            ...
            >
              ...
              {question.variants.map((variant, ind) => {
                ...
                if (variant.correct) optionColor = 'bg-green-500'
                return (
                  <div
                    key={`option${index}-${ind}`}
                    className="flex items-center gap-1"
                  >
                    <i>
                      <div
                        className={`${optionColor} w-2 h-2 rounded-full`}
                      ></div>
                    </i>
                    {variant.text}
                  </div>
...
function Switch() {
  return (
    <div className="w-full flex flex-wrap">
      ...
      <label
        htmlFor="createTabOpened"
        className="cursor-pointer pointer-events-none bg-[#5348F2] text-center w-1/2 peer-checked:bg-[#282828] peer-checked:pointer-events-auto select-none"
      >
        Create question
      </label>
      <label
        htmlFor="createTabOpened"
        className="cursor-pointer bg-[#282828] text-center w-1/2 pointer-events-auto peer-checked:bg-[#5348F2] peer-checked:pointer-events-none select-none"
      >
        Generate question
      </label>
...
function CreateQuestionBlock() {
  ...
  function addQuestions() {
    ...
    if (
      ...
      inputValues.find((o) => o.text === '') != undefined
    ) {
  ...
  return (
    <>
      ...
      <div className="flex">
        <input
          id="questionInput"
          className="flex-1 px-2 bg-[#282828] text-[#91898C]"
      ...
      <div className="flex flex-col w-full gap-2 mt-2">
        {optionInputs.map((optionInput, index) => {
          ...
          return (
            <div
              key={`option${optionInput.id}`}
              className="flex w-full relative"
            >
              <button
                onClick={() => {
                  let temp = [...optionInputs]
                  temp[index].correct = !temp[index].correct
                  setOptionInputs(temp)
                }}
                className={`p-2 text-white w-16 ${optionColor}`}
              >
                {text}
              </button>
              <input
                type="text"
                id={`optionInput${optionInput.id}`}
                className="flex-1 px-2 bg-[#282828] text-[#91898C]"
              />
              <button
                onClick={() => {
                  setOptionInputs((optionInputs) =>
                    optionInputs.filter((s, i) => i != index)
                  )
                }}
      ...
      <Button
        disabled={optionInputs.length >= 4}
        onClick={addOption}
        className={`${styles.dropdown} w-full h-10`}
      >
        + Add option
      </Button>
      <Button
        onClick={addQuestions}
        className={`${styles.button_success} p-2 rounded-r-md mt-10 w-full`}
      >
        Create question
      </Button>
    </>
...
function GenerateQuestionBlock() {
  ...
  return (
    <>
      ...
      <div className="flex">
        <Dropdown
          className={`h-auto rounded-r-none ${styles.dropdown}`}
          menu={{ items }}
          trigger={['click']}
        >
        ...
        <Button
          className={`py-2 px-3 rounded-l-none flex-1 text-start text-sm ${styles.button_success}`}
        >
          Generate question
        </Button>
      </div>
