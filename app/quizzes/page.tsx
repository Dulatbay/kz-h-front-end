  ...
  return (
    <div className="mt-10 w-full max-w-[1200px] min-w-80 mx-auto flex flex-col gap-6 px-8">
      ...
      <div className="flex flex-wrap w-full gap-2">
        <Dropdown
          onSelect={(tag) =>
            toggleTag({ type: 'topics', tag, query_value: `"${tag}"` })
          }
          title="Topics"
        ...
        <Dropdown
          onSelect={(tag) =>
            toggleTag({ type: 'level', tag, query_value: tag.toUpperCase() })
          }
          title="Difficulty"
          ...
          return (
            ...
            >
              ...
              >
                ...
                >
                  <path
                    d="M1.00024 1L4.35343 4.35319"
                    stroke="white"
                    strokeLinecap="round"
                  />
                  <path
                    d="M1 4.35303L4.35319 0.999839"
                    stroke="white"
                    strokeLinecap="round"
                  />
                </svg>
      ...
      <div className="flex flex-col">
        <div className="flex gap-3">
          ...
          <select
            onChange={(lang) => setLanguage(lang.target.value.split(' ')[0])}
            className="bg-[#FFFFFF24] text-center px-1 py-2 rounded-md"
          >
            ...
            {quizzes.map((row, i) => {
              ...
              return (
                <tr key={'row' + i} className="odd:bg-zinc-800">
                  <td>{row.status ? <SolvedMark /> : <></>}</td>
                  <td>
                    <a href={`/quizzes/${row.id}/preview`}>{row.title}</a>
                  </td>
                  <td>{43}%</td>
...
function Dropdown({
  ...
  return (
    <select
      className="bg-[#FFFFFF24] flex-1 md:max-w-36 p-3 overflow-visible rounded-md cursor-pointer"
      value={title}
      onChange={(e) => onSelect(e.target.value)}
    >
      <option className="bg-zinc-800" disabled>
        {title}
      </option>
      {options.map((name, i) => {
        return (
          <option
            className="bg-zinc-800 hover:bg-slate-300 cursor-pointer"
            key={title + i}
          >
            {name}
          </option>
...
function SearchBar({
  ...
  return (
    <div className="flex flex-1 min-w-48">
      <input
        className="bg-[#FFFFFF24] w-full text-[#91898C] rounded-lg pl-3"
        type="text"
        placeholder="Search"
        onChange={onSearch}
      />
...
function PickOne() {
  ...
  async function pickRandom() {
    await fetch(`${process.env.API_URL}/quizzes/random`)
  ...
  return (
    <button onClick={pickRandom} className="flex gap-2 items-center px-2">
      ...
      <h3 className="text-[#2CBB5D] text-md text-nowrap max-[400px]:hidden">
        Pick one
      </h3>
    </button>
  )
